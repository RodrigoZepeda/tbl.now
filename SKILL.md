...
- `units =` — the shared default for `event_units`, `report_units` and
  `revision_units`. Say `units = "days"` once instead of three times;
...
`"retracted"`, `"pending"` and `"unknown"` filter to one outcome;
`"net"` is confirmed − retracted; `"by_type"` returns one row per
outcome. On an object with no revision process anything but `"total"`
warns and pools.
...
- **A pending case never appears.** It has no revision date, so it has
...
- **`delay` is measured from the EVENT**, not from the report, so
  `get_nth_reported_cases(tn, 7)` and `get_nth_revised_cases(tn, 7)`
  describe the same seven days. `.revision_delay` (report → resolution)
  is a different quantity.
- **An empty selection is an error, not a zero-row object.** Nothing
  revised yet, or no case with that outcome, aborts with the reason
  named.
- The result carries all three dates. `revision_type` on it is the
...
Covariates are an **as-of promise**: a nowcast may only use values
available at the fitted object’s `now`. In backtests, rebuild or join
covariates per snapshot, or pass explicit forecast covariates only when
the engine is meant to use them that way.
...
expression evaluated in the data (`.delay` and `.revision_delay` are
visible); `NA` is **not** a match. Existing flags are merged, never
cleared, and the flag column is created as `.is_censored_report` /
`.is_censored_revision` when there is none. Replacing a date rebuilds
...
**Pending cases are skipped by the revision verbs.** `"pending"` means
reported and still waiting, so the case has no revision date; writing
...
`is_censored_revision` is the exact twin of `is_censored_report`, one
axis over: it marks rows whose **revision delay** (report → resolution)
is a bound rather than a measurement. It requires a `revision_date` —
there is no revision delay to bound without one.
...
`revision_type` may hold **only** `"confirmed"`, `"retracted"`,
`"pending"` or `NA`. Anything else is an error at construction and at
every rebuild.

`revision_levels` is how data recorded in other words gets into those
...
- [`add_temporal_effects()`](https://rodrigozepeda.github.io/tbl.now/reference/add_temporal_effects.md)
  ...
  `date_type = "event_date"`, `"report_date"`, and `"revision_date"`
  when the object has a revision process.
...
- **The spec knows about the grid.**
  [`aggregate_time_units()`](https://rodrigozepeda.github.io/tbl.now/reference/aggregate_time_units.md)
  coarsens it along with the dates: `day_of_week` / `weekend` /
  `day_of_month` / `holiday_lags` / `weekend_lags` survive only
  `to = "days"`, `week_of_year` only down to `"weeks"`, `month_of_year`
  only down to `"months"`. `seasons` are **rescaled** (a 365-day period
  becomes 52.14 weeks) and dropped once the new period is two units or
  shorter. A spec left with nothing is removed.
- **`holidays` survive any grid.** On days the holiday column is the
  usual `0/1` indicator; on a coarser grid it is the **share of the
  period’s days that are holidays** (`1/7` for a week containing
  Christmas Day, `1/31` for December).
  [`compute_temporal_effects()`](https://rodrigozepeda.github.io/tbl.now/reference/add_temporal_effects.md)
  reads the units off the object, so this is automatic — nothing extra
  to call.
...
10. **Revision calendar effects** (`"revision_weekday"`,
    `"revision_week"`, `"revision_month"`, `"revision_holiday"`,
    `"revision_holiday_lag"`) — revision-date twins for resolved cases.
    These appear in
    [`autoplot()`](https://ggplot2.tidyverse.org/reference/autoplot.html)
    when the object has a revision process.
11. **Revision periodicity periodogram** (`"revision_seasonality"`) —
    cycles in revision arrivals.
...
Every plot takes `palette =`, and every colour in it is named for the
**role** it plays, not for the hue it happens to be. Override the roles
you care about; the rest keep the package defaults, so a partial palette
is always complete.
...
A palette handed in as a bare vector must carry **every** role; a
missing one is an error listing what is absent, rather than a silent
fall back to a colour you did not choose.

### Sizes: `size` and `linewidth`

`size` and `linewidth` are **multipliers**, default `1`, so they scale a
panel’s marks without flattening its hierarchy — `size = 2` doubles both
the small and the large points rather than making them equal.
...
A grid the package draws itself — not `ggplot2`’s panel grid, which
these plots switch off — has its own **absolute** argument:
...
is the exception: its `size` is an absolute point size in millimetres,
because the marks are millimetres and the lattice is data units, so no
default suits every cell count and figure size. Raise it until the
...
draw only bars, tiles or areas, so they take neither.
...
`n` and `total` are **different questions** and the print method now
says which per block: `n` counts the block’s own kind of thing (dates on
the grid for `cases`, runs for `zero_run`, **(event, report) cells** for
`delay` and `composition`) and `total` always counts **cases**. `prop`
is computed from `total`.
...
report**; `event_to_revision` is measured from the event, so it is
...
| `duplicates` | rows repeating on the full key (including the revision columns). Defaults **on** here, unlike [`validate_tbl_now()`](https://rodrigozepeda.github.io/tbl.now/reference/validate_tbl_now.md) |
...
- **It never runs a test.** Drift and batching are statements about a
  *distribution*; answering them means choosing a method, a window and a
  multiplicity correction. Those questions are not in
  [`diagnose()`](https://rodrigozepeda.github.io/tbl.now/reference/diagnose.md)
  at all – call `diagnose_drift(x, axis =)` and
  `diagnose_batches(x, axis =)` yourself.
...
- **[`diagnose_batches2()`](https://rodrigozepeda.github.io/tbl.now/reference/diagnose_batches2.md)**
  ...
  batch’s own deficit sits there). Renamed from `diagnose_batch_shape()`
  in v0.33.0.
...
or revisions which arrived after the nowcast’s `now`), or `NULL` to
reuse the nowcast’s own source data. Observed counts are defined by
`truth_axis` and `truth_type`: report totals by default, or
revision-axis quantities such as confirmed/net when requested. There is
**no `observed_col`** (removed 0.27.0): the observed counts are read off
the object with
...
- **[`tidy()`](https://rodrigozepeda.github.io/tbl.now/reference/tidy.nowcast.md)
  ...
  `now` date, target) with **both halves of the comparison**: the
  retrospective prediction (`estimate`, `conf.low`, `conf.high`,
  `level`, named as everywhere else), what was eventually `observed`,
  and the scores (`wis`, `ae_median`, `coverage_50`, `coverage_90`) —
  ready for `dplyr` or `ggplot2`. `level` is one number for the whole
  table, since a backtest refuses engines whose `quantile_levels`
  disagree.
...
- **`aggregate_time_units` / sparse daily data:** moves every date onto
  ...
  and updates `event_units` / `report_units` / `revision_units` so
  ...
  which axes move (`"all"`, `"event"`, `"report"`, `"revision"`);
  ...
  want — weeks do not nest inside months. Materialised temporal-effect
  columns are dropped and the lazy spec is coarsened with the dates —
  see the temporal-effects skill above for what survives which grid.
...
- `diagnose_batches2(at =)` needs `at` to be on the **report grid**; a
  date that carries no rows is reported as **zero arrivals**, not an
  error (v0.33.0). The volume screen
  [`diagnose_batches()`](https://rodrigozepeda.github.io/tbl.now/reference/diagnose_batches.md)
  scans them all.
- The whole batch family **ignores censored arrival dates**
  (`drop_censored = TRUE`, v0.33.0): a censored date is a bound, not an
  arrival.
- [`diagnose_drift()`](https://rodrigozepeda.github.io/tbl.now/reference/diagnose_drift.md)
