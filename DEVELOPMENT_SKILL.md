...
`vignettes/articles/nowcast-comparison.rds`; `run_models` is `FALSE` in
the article so the build never fits anything. Add your engine there,
...
- Guard optional packages with `skip_if_not_installed()`, and heavy work
  with `skip_on_cran()`.
...
- Prefer mocking to fitting. A
  [`tidy()`](https://rodrigozepeda.github.io/tbl.now/reference/tidy.nowcast.md)
  method can be tested against `local_mocked_bindings()` instead of
  compiling a Stan model; verify against a real fit once, by hand, and
...
- **A vignette that displays cached results has TWO sources of truth.**
  The code on the page and the script that produced the cache must
  agree, or the reader cannot reproduce what they see. Whenever you
  change a fitting call in one, change it in the other, and re-run the
  precompute.
...
- **One [`set.seed()`](https://rdrr.io/r/base/Random.html) at the top of
  ...
  anything did. `nowcast_comparison.R` does this in `run_engine()`; both
  engines then reproduce to `max abs diff == 0` with the RNG
...
[`as_tbl_now()`](https://rodrigozepeda.github.io/tbl.now/reference/as_tbl_now.md)
method, a round-trip test, a
[`tidy()`](https://rodrigozepeda.github.io/tbl.now/reference/tidy.nowcast.md)
method (unless it returns only delays), a censoring-collapse call, and a
vignette section covering both the plain and the stratified case.
