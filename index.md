...
draws a rolling fan chart of the delay distribution, while
...
test for a gradual trend or an abrupt change (with an
autocorrelation-robust test, since a delay series is correlated with
itself).
...
flags them — and, crucially, tells them apart from epidemic peaks (which
also spike the volume but keep the normal *short* delays) by
additionally requiring an anomalous *delay* signal:
...
shows the reporting-volume and mean-delay timelines with the flagged
dates marked, so you can see *why* each was flagged:
...
![](reference/figures/README-batches-plot-1.png)

## Extreme delays

Extreme delays can be censored with the
[`censor_delays_above()`](https://rodrigozepeda.github.io/tbl.now/reference/censor_delays_above.md)
function that assigns upper bounds to the delay. In the following data
frame for example, an extreme delay of 300 is marked as `censored` with
that function:
