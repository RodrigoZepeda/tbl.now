...
| ![revision](reference/figures/revision_type.svg) | `revision_type` | What the revision date resolved to. Only `confirmed`, `retracted`, `pending` or `NA` are ever stored; use `revision_levels` for data recorded in other words. *Optional*. |
| ![revision_levels](reference/figures/revision_type.svg) | `revision_levels` | A named dictionary translating the labels in `revision_type` into those four, e.g. `c(positive = “confirmed”)`. *Optional*. |
...
| ![is_censored_revision](reference/figures/censoring.svg) | `is_censored_revision` | The same on the revision axis: flags rows whose *revision* delay is a bound rather than a measurement. *Optional*. |
