...
test_that("tbl_format_footer.tbl_now displays mandatory attributes when mutating / filtering / grouping / etc", {
  ...
  result <- dfnow %>% filter(sex == "M") %>% filter(region == "North")
  ...
  result <- dfnow %>% mutate(new_var = 2) %>%  mutate(other_var = 3)
  ...
  result <- dfnow %>% group_by(region) %>% group_by(sex, .add = TRUE)
  ...
  result <- dfnow %>% group_by(region) %>% filter(sex == "M")
  ...
  result <- dfnow %>% group_by(region) %>% mutate(new_var = 1)
  ...
  #Now apply mutate and test after grouping and mutate twice
  result <- dfnow %>% group_by(region) %>% mutate(new_var = 1) %>% mutate(new_col = "a")
  ...
  result <- dfnow %>% group_by(region) %>% ungroup()
