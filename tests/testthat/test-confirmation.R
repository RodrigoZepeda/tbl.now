...
#   * `now` left behind a confirmation that has already happened,
...
test_that("a confirmation process is recorded on the object", {
  flu <- flu_fixture()

  expect_true(has_confirmation(flu))
  expect_equal(get_confirmation_date(flu), "result")
  expect_equal(get_confirmation_type(flu), "outcome")
  expect_equal(get_confirmation_units(flu), "days")

  # The generated pair, on the same anchor as the other numeric columns.
  expect_true(all(c(".confirmation_num", ".confirmation_delay") %in% colnames(flu)))
  resolved <- !is.na(flu$result)
  expect_equal(
    flu$.confirmation_delay[resolved],
    (flu$.confirmation_num - flu$.report_num)[resolved]
  )

  # Protected, so removing one downgrades the object -- as `.delay` does.
  expect_true(all(
    c(".confirmation_num", ".confirmation_delay", "result", "outcome") %in%
      get_protected_cols(flu)
  ))
})

test_that("an object with no confirmation is unchanged", {
  plain <- tbl_now(
    data.frame(
  ...
  expect_false(has_confirmation(plain))
  expect_null(get_confirmation_date(plain))
  expect_null(get_confirmation_units(plain))
  expect_false(any(
    c(".confirmation_num", ".confirmation_delay") %in% colnames(plain)
  ))
})

test_that("a confirmation date with no outcome is NA, not a guess", {
  cases <- data.frame(
  ...
  expect_warning(
    x <- tbl_now(cases,
      event_date = "e", report_date = "r", confirmation_date = "cf",
      data_type = "linelist", verbose = FALSE
    ),
    "cannot say whether"
  )
  expect_true(all(is.na(x[[get_confirmation_type(x)]])))
})

test_that("a case with no confirmation date is pending", {
  cases <- data.frame(
    e = as.Date("2021-01-04") + 0:2,
    r = as.Date("2021-01-05") + 0:2,
    # Only ONE confirmation: a single date has no spacing to infer a grid from,
  ...
  outcome <- x[[get_confirmation_type(x)]]
...
test_that("an unrecognised outcome is refused", {
  ...
  expect_error(
    tbl_now(cases,
      ...
      confirmation_date = "cf", confirmation_type = "ty",
      data_type = "linelist", verbose = FALSE
    ),
    "unrecognised"
  )
...
test_that("an outcome without a date is refused", {
  ...
  expect_error(
    tbl_now(cases,
      event_date = "e", report_date = "r", confirmation_type = "ty",
      data_type = "linelist", verbose = FALSE
    ),
    "without a"
  )
...
test_that("a confirmation moves `now` forward", {
  cases <- data.frame(
  ...
  # A confirmation is an observation: the as-of moment is the last thing
  # anybody knew, not the last thing anybody reported.
  expect_equal(get_now(x), as.Date("2021-01-11"))
})

test_that("a `now` behind the last confirmation is refused", {
  flu <- flu_fixture()
  broken <- flu
...
test_that("the timeline is checked, not assumed", {
  cases <- data.frame(
    ...
    cf = as.Date("2021-01-06") + 0:2, # confirmed BEFORE reported
    ty = rep("confirmed", 3)
  )
  expect_warning(
    tbl_now(cases,
...
test_that("the confirmation attributes survive dplyr verbs", {
  flu <- flu_fixture()
  ...
  for (name in names(verbs)) {
    ...
    expect_equal(
      get_confirmation_date(result), "result",
      label = paste0(name, " keeps confirmation_date")
    )
    expect_equal(
      get_confirmation_type(result), "outcome",
      label = paste0(name, " keeps confirmation_type")
    )
    expect_equal(
      get_confirmation_units(result), "days",
      label = paste0(name, " keeps confirmation_units")
    )
  }
})

test_that("tbl_now_attributes() lists the confirmation attributes", {
  ...
  expect_true(all(
    c("confirmation_date", "confirmation_type", "confirmation_units") %in% listed
  ))
})
...
test_that("add / change / remove round-trip", {
  ...
  added <- add_confirmation(plain, cf, ty)
  expect_true(has_confirmation(added))
  expect_equal(get_confirmation_date(added), "cf")

  # Adding twice is a mistake worth naming.
  expect_error(add_confirmation(added, other), "already has")

  changed <- change_confirmation(added, other, ty)
  expect_equal(get_confirmation_date(changed), "other")

  removed <- remove_confirmation(changed)
  expect_false(has_confirmation(removed))
  expect_false(any(
    c(".confirmation_num", ".confirmation_delay") %in% colnames(removed)
  ))
...
test_that("the counting getters refuse an object with no confirmation", {
  plain <- tbl_now(
    data.frame(e = as.Date("2021-01-04") + 0:2, r = as.Date("2021-01-05") + 0:2),
    event_date = "e", report_date = "r", data_type = "linelist", verbose = FALSE
  )
  expect_error(get_latest_confirmed(plain), "needs a confirmation process")
  expect_error(get_net_confirmed(plain), "needs a confirmation process")
})
...
test_that("unusable delays are dropped and counted", {
  cases <- data.frame(
    ...
    # Two rows are confirmed BEFORE they were reported: a negative delay.
...
test_that("plot_confirmation_delay() draws the reporting process", {
  skip_if_not_installed("ggplot2")
  expect_s3_class(plot_confirmation_delay(flu_fixture(n_days = 20L)), "ggplot")
})

test_that("complete_zeroes() extends the grid to the confirmation-aware now", {
  counts <- data.frame(
  ...
  x <- suppressWarnings(tbl_now(counts,
    event_date = "e", report_date = "r", case_count = "n",
    confirmation_date = "cf", data_type = "count-incidence", verbose = FALSE
  ))
...
test_that("update() keeps the confirmation process and de-duplicates on it", {
  # `update.tbl_now()` de-duplicates on everything EXCEPT the generated
  # columns. Adding `.confirmation_num`/`.confirmation_delay` to that generated
  ...
  expect_true(has_confirmation(same))
  expect_equal(get_confirmation_date(same), "cf")
  expect_equal(get_confirmation_type(same), "ty")
...
confirmation_axis_fixture <- function(seed = 7L) {
  set.seed(seed)
...
test_that("batch_test() finds a laboratory backlog only on the confirmation axis", {
  ...
  expect_equal(sum(on_confirmation$batch, na.rm = TRUE), 1L)

  flagged <- on_confirmation[which(on_confirmation$batch), ]
...
test_that("the confirmation axis needs a confirmation process", {
...
test_that("pending cases are excluded from the confirmation axis", {
  # A pending case has no confirmation date, so counting it would invent an
  # arrival on a date it does not have.
  x <- flu_fixture(n_days = 20L)
  increments <- tbl.now:::.batch_report_increments(x, axis = "confirmation")

  resolved <- sum(!is.na(x[[get_confirmation_date(x)]]))
  expect_equal(sum(increments$.count), resolved)
  expect_lt(resolved, nrow(x))
})

test_that("the reporting-process plots accept the confirmation axis", {
  skip_if_not_installed("ggplot2")
  x <- confirmation_axis_fixture()
  ...
  for (fn in c("plot_reporting_process", "plot_epidemic_process",
    ...
    drawn <- suppressWarnings(suppressMessages(
      do.call(fn, list(x, axis = "confirmation"))
    ))
...
test_that("get_initial_confirmed() and get_nth_confirmed() count by delay", {
  cases <- data.frame(
    ...
    # Confirmation delays of 0, 2, 1, 90 and 2 periods.
...
test_that("censor_confirmation_delays_above() returns stragglers to pending", {
  cases <- data.frame(
    onset = as.Date("2021-01-04") + 0:4,
    visit = as.Date("2021-01-05") + 0:4,
  ...
  censored <- suppressMessages(censor_confirmation_delays_above(flu, 30))
  ...
  expect_true(is.na(censored[[".confirmation_delay"]][4]))
  # ...and the confirmed count falls by exactly that one case.
  expect_equal(sum(get_latest_confirmed(censored)[["n"]]), 4)
})

test_that("plot_confirmation_status() shows the resolution front", {
  skip_if_not_installed("ggplot2")
  x <- flu_fixture(n_days = 20L)

  expect_s3_class(plot_confirmation_status(x), "ggplot")
  expect_s3_class(plot_confirmation_status(x, proportion = FALSE), "ggplot")

  plain <- tbl_now(
    data.frame(e = as.Date("2021-01-04") + 0:4, r = as.Date("2021-01-05") + 0:4),
    event_date = "e", report_date = "r", data_type = "linelist", verbose = FALSE
  )
  expect_error(plot_confirmation_status(plain), "needs a confirmation process")
})

test_that("align_weeks() aligns the confirmation date too", {
  weekly <- data.frame(
  ...
  # them, leaving `.confirmation_delay` fractional.
  expect_false(all(x$.delay == round(x$.delay)))
  expect_false(all(x$.confirmation_delay == round(x$.confirmation_delay)))

  aligned <- suppressWarnings(suppressMessages(align_weeks(x)))

  expect_true(has_confirmation(aligned))
  expect_true(all(aligned$.delay == round(aligned$.delay)))
  expect_true(all(aligned$.confirmation_delay == round(aligned$.confirmation_delay)))
})

test_that("the delay family measures to the confirmation when asked", {
  ...
  on_confirmation <- tbl.now:::.tbl_now_delay_long(x, NULL, axis = "confirmation")

  expect_equal(stats::median(rep(on_report$delay, on_report$weight)), 1)
  expect_equal(stats::median(rep(on_confirmation$delay, on_confirmation$weight)), 3)
  # Measured from the event on both axes, so the confirmation delay is never
  # the shorter of the two.
  expect_gt(sum(on_confirmation$delay * on_confirmation$weight),
            sum(on_report$delay * on_report$weight))
})

test_that("the delay diagnostics accept the confirmation axis", {
  skip_if_not_installed("ggplot2")
  ...
  expect_s3_class(quietly(plot_reporting_triangle(x, axis = "confirmation")), "ggplot")
  expect_s3_class(quietly(plot_delay_profiles(x, axis = "confirmation")), "ggplot")
...
test_that("the confirmation triangle is labelled as one", {
  skip_if_not_installed("ggplot2")
  x <- confirmation_axis_fixture()

  drawn <- suppressWarnings(suppressMessages(
    plot_reporting_triangle(x, axis = "confirmation")
  ))
  expect_equal(drawn$labels$title, "Confirmation triangle")
})

test_that("simulate_batch() carries the confirmation through", {
  set.seed(3)
  ...
  expect_true(has_confirmation(sim))
  expect_equal(get_confirmation_date(sim), "result")
