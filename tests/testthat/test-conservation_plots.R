...
test_that("a planted batch is confirmed by the screening machinery", {
  ...
  batched  <- quiet_messages(suppressWarnings(simulate_batch(tn, closed_dates = closed)))
  screened <- quiet_messages(suppressWarnings(diagnose_batches(batched, lookback = 6)))
