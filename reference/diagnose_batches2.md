...
- at:
  ...
  [`diagnose_batches()`](https://rodrigozepeda.github.io/tbl.now/reference/diagnose_batches.md).
  A date on the report grid that carries no rows is reported as zero
  arrivals, not an error.
...
- axis:
  ...
  is. `"revision"` needs a revision process (see
...
a zero, so a report date on which nothing arrived has no rows at all;
that is the observation "no arrivals", not a missing one, and the test
reports zero arrivals for it instead of aborting. Only a date off the
object's report grid is an error.
...
#> Warning: ! `diagnose_batches2()` is experimental: results are not guaranteed and the
