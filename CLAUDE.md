...
`tbl.now` provides the `tbl_now` S3 class — an extension of `tibble`
designed for epidemiological nowcasting. It stores **two time indices
simultaneously** (`event_date` and `report_date`) together with
modelling metadata (strata, covariates, temporal effects, etc.) and
...
can be called multiple times; each call appends a new spec to the list.
Multiple calls with different `date_type` values are supported.
...
This calls the underlying
[`add_temporal_effects.data.frame()`](https://rodrigozepeda.github.io/tbl.now/reference/add_temporal_effects.md)
for each stored spec and records the created column names in the
`computed_temporal_effect_cols` attribute.
...
`T. effects (lazy): [event_date] week_of_year, month_of_year` — before
computation - `T. effects: [event_date] week_of_year, month_of_year` +
`T. effect cols: ".event_week_of_year" ...` — after computation
...
all other dplyr verbs **preserve the `temporal_effects` spec
unchanged**. They never trigger computation. Only
[`compute_temporal_effects()`](https://rodrigozepeda.github.io/tbl.now/reference/compute_temporal_effects.md)
adds columns.

If a **computed** column is removed via
[`select()`](https://dplyr.tidyverse.org/reference/select.html), it is
dropped from `computed_temporal_effect_cols` but the spec in
`temporal_effects` is preserved.
...
the object to a plain tibble with a warning.

Removing a **strata** or **covariate** column silently drops that column
from the attribute.
...
- **Do not call
  ...
  before
  [`add_temporal_effects()`](https://rodrigozepeda.github.io/tbl.now/reference/add_temporal_effects.md)**
  — if no spec is stored the call is a no-op.
- **[`get_temporal_effects()`](https://rodrigozepeda.github.io/tbl.now/reference/nowcast_data_getters.md)
  returns a list of specs** (not column names). Use
  [`get_temporal_effect_cols()`](https://rodrigozepeda.github.io/tbl.now/reference/nowcast_data_getters.md)
  for column names.
- **Reconstruction via
  [`tbl_now()`](https://rodrigozepeda.github.io/tbl.now/reference/tbl_now.md)
  inside group_by/ungroup/summarise** does NOT pass `t_effects`; the
  spec is copied manually afterward. This is intentional.
- **`replace_temporal_effects(x, t_effects)`** removes computed columns
  before storing the new spec — call
  [`compute_temporal_effects()`](https://rodrigozepeda.github.io/tbl.now/reference/compute_temporal_effects.md)
  again afterward.
- **Week-aligned data**: use `align_weeks = TRUE` in
  [`tbl_now()`](https://rodrigozepeda.github.io/tbl.now/reference/tbl_now.md)
  or call
  [`align_weeks()`](https://rodrigozepeda.github.io/tbl.now/reference/align_weeks.md)
  explicitly to ensure integer `.delay` values for weekly data.
