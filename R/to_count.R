...
to_count.tbl_now <- function(x, to = NULL, ...) {
  ...
  if (get_data_type(x) == "count-incidence" & to == "count-incidence") {
    ...
    x <- x |>
      ...
      dplyr::group_by(dplyr::across(dplyr::all_of(c(get_event_date(x), get_is_censored(x), .validation_group_cols(x), get_strata(x), get_temporal_effect_cols(x), get_covariates(x))))) |>
  ...
  x <- x |>
    dplyr::arrange(dplyr::across(dplyr::all_of(c(get_event_date(x), get_strata(x), get_is_censored(x), .validation_group_cols(x), get_covariates(x), get_temporal_effect_cols(x)))))
